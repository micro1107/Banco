 ...
 class Cpdf
{
    ...
    protected function openFont($font) {
		...
		*/
		...
		//$this->tempPath = './fonts';
		//print "<br>ESTE ES EL PATH = ".$this->tempPath."<BR>";
		
        // $fp = fopen($this->tempPath.'/'.$cachedFile,'w'); // use the temp folder to write cached font data
		/* R&OS php pdf class font cache file */
		/* fwrite($fp,'<?php return '.var_export($cachedFont,true).'; ?>'); */
        // fclose($fp);
    ...
    public function addText($x, $y, $size, $text, $width = 0, $justification = 'left', $angle = 0, $wordSpaceAdjust = 0, $test=0) {
        ...
        foreach($directives as  $pos => $directive){
            ...
            if($func == 'defaultFormatting'){ // default font style, bold, italic, ...
				...
				//print "<br><BR>TEXT= ".$text;
				//print "<BR>DIRECTIVA = ".$directive['p']."<br>POS= ".$pos."<BR><BR>";
				// 2014-SEP-14: SE QUITA LA DIRECTIVA QUE SIEMPRE DISMINUYE EN 1 LA POSICION INCIAL, SE INCLUYE SENTENCIA TRIM PARA DEVOLVER CADENA SIN ESPACIOS
                return trim( mb_substr( $text,$pos + $directive['p'], $len, 'UTF-8') );
