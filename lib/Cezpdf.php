    ...
    protected function ezTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$gap,$size,&$y,$optionsAll=array()){
        ...
        while ($ok==0){
            foreach ($cols as $colName=>$colHeading){
                ...
                $this->ezText( utf8_encode($colHeading) ,$size,array('aleft'=> $pos[$colName],'aright'=>($maxWidth[$colName]+$pos[$colName]),'justification'=>$justification));
                $dy = $y-$this->y;
                if ($dy>$mx){
                    $mx=$dy;
                }
                            ...
                            foreach ($lines as $line){
                                ...
                                while (strlen($line) || $start){
                                    ...
                                    if ($colNewPage){
										...
										// 2014-SEP-14 SE INCLUTE CONDICIONAL PARA CONVERSION A UTF8 
										//             (APLICA SOLO PARA LA PRIMERA LINEA, SI ES UN TEXTO LARGO DE LA CELDA QUE SE PARTE EN VARIAS LINEAS)
